...
Chosen option: **Option 1 — Rust + `reqwest`, raw HTTP**, because it is the only option that keeps the key in Rust, sidesteps CORS entirely, and avoids coupling to SDK release cycles. Each provider is a module in `src-tauri/src/providers/` implementing a one-method `Provider` trait, **`stream(...)`**, which streams text deltas over a Tauri `Channel<StreamDelta>` and returns the accumulated `{content, model}`. SSE is parsed by a shared, UTF-8-safe line driver (`for_each_sse_data` in `providers/mod.rs`). `providers::stream` dispatches local **Ollama by `provider` id**; every other provider by the wire **`protocol`** on the request (`"anthropic"`/`"gemini"` → the native module, anything else → the shared OpenAI engine `openai::chat_completions_stream` against the request's `base_url`). Per-provider quirks (Anthropic `system` + `anthropic-version`; Gemini `model` role + `x-goog-api-key`) live in their own modules.

### Consequences

* **Positive:** The key stays in Rust, there is no CORS, and there is no SDK version drift. The command surface is a single call, `chat_stream(provider, model, messages, on_delta)` (`commands/chat.rs`). Adding a **new wire protocol** is mechanical: a module implementing `Provider::stream` + a `protocol` arm in `providers::stream` + the value in `ProviderProtocol` (`src/lib/db.ts`). Most providers need no new code at all — OpenAI-compatible services (OpenAI, Mistral, Groq, OpenRouter, DeepSeek, …) are just `openai`-protocol presets sharing `openai::chat_completions_stream`.
