...
impl TrayIcon {
    pub fn new(id: TrayIconId, attrs: TrayIconAttributes) -> crate::Result<Self> {
        ...
        // Use only the file stem (no dir, no .png) so KDE Plasma's SNI host can
        // resolve the icon name relative to the theme path. Passing the full
        // absolute path breaks KDE because StatusNotifierItem IconName must be
        // a freedesktop icon name (no slashes).
